...
const templateOpts = [
  { value:'t1', label:'门店基础巡检' },
  { value:'t2', label:'明星店铺综合巡检' },
  { value:'t3', label:'陈列展示巡检' },
]
...
const inspectionItems = reactive([
  { itemId:'i1',categoryName:'环境卫生',itemName:'地面是否整洁',problemDesc:'',standard:'地面无污渍、无杂物、无水渍，保持光洁。',screenshots:[],result:'pass',images:['img1','img2'] },
  { itemId:'i2',categoryName:'环境卫生',itemName:'设备是否整洁',problemDesc:'',standard:'设备表面无灰尘、无油污，定期清洁保养。',screenshots:[],result:'pass',images:[] },
  { itemId:'i3',categoryName:'行为规范',itemName:'上班是否手机',problemDesc:'',standard:'工作期间禁止使用手机进行非工作活动。',screenshots:[],result:'pass',images:['img1'] },
])

const filteredItems = () => {
  if (!selectedTemplate.value) return []
  return inspectionItems.filter(i => i.categoryName === categories[Number(activeCategory.value)])
}

const onTemplateChange = () => {
  if (inspectionItems.some(item => item.result === 'fail' || item.problemDesc.trim())) {
    message.warning('切换模板将清空当前检查结果')
  }
}
...
const submitSpotCheck = () => {
  if (!selectedTemplate.value) { message.warning('请选择巡检模板'); return }
  const items = filteredItems()
  if (items.length === 0) { message.warning('请先选择巡检模板'); return }
  const allJudged = items.every(i => i.result === 'pass' || i.result === 'fail')
  ...
  setTimeout(() => { submitting.value = false; message.success('点检提交成功'); router.back() }, 1000)
}
</script>
...
<template>
  <div class="exe-page">
    <div class="exe-header">
      ...
      <span class="exe-header-label">视频点检</span>
      <span class="exe-header-divider">|</span>
      <span class="exe-header-store">{{ storeName }}</span>
    </div>
    ...
    <div class="exe-body">
      <div class="exe-video" ref="videoRef">
        <div class="exe-cam-tabs">
          ...
          <a-popover trigger="click" placement="bottomRight" v-model:open="camDropdownOpen" overlay-class-name="cam-drop-popover">
            <span class="exe-cam-more">全部 <DownOutlined style="font-size:10px" /></span>
            <template #content>
      ...
      <div class="exe-panel">
        ...
        <a-select v-model:value="selectedTemplate" :options="templateOpts" placeholder="请选择巡检模板" style="width:100%" size="large" allow-clear @change="onTemplateChange" />
        <a-tabs v-if="selectedTemplate" v-model:activeKey="activeCategory" size="small" class="exe-cats">
          <a-tab-pane v-for="(cat, i) in categories" :key="String(i)" :tab="cat" />
        </a-tabs>
        <div v-if="selectedTemplate" class="exe-items">
          <div v-for="item in filteredItems()" :key="item.itemId" class="exe-item">
            <div class="exe-item-title">{{ item.itemName }}</div>
            <a-textarea v-model:value="item.problemDesc" placeholder="如有问题请输入" :rows="2" :maxlength="200" @input="onProblemInput(item)" />
        ...
        <div v-else class="exe-empty">请选择巡检模板</div>
        <div v-if="selectedTemplate" style="margin-top:12px">
          <a-select v-model:value="selectedAssignee" :options="assigneeOpts" placeholder="问题指派" style="width:100%" size="large" allow-clear />
        </div>
        <a-button type="primary" block size="large" :loading="submitting" @click="submitSpotCheck" style="margin-top:16px">完成点检</a-button>
      </div>
    </div>
...
.exe-header-store { font-size: 15px; font-weight: 600; color: #1a3353; }
.exe-body { display: flex; flex: 1; overflow: hidden; }
.exe-video { flex: 1; display: flex; flex-direction: column; padding: 12px; min-width: 0; }
.exe-cam-tabs { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; padding: 4px 0; flex-wrap: nowrap; overflow: hidden; }
.exe-cam-chips { flex: 1; min-width: 0; display: flex; align-items: center; gap: 8px; overflow: hidden; }
.exe-cam-arrow { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 50%; cursor: pointer; color: #666; font-size: 12px; background: #fff; border: 1px solid #e8e8e8; transition: all .2s; flex-shrink: 0; }
...
.exe-cam-chip { display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 14px 6px 10px; border-radius: 20px; cursor: pointer; white-space: nowrap; font-size: 13px; color: #555; background: #fff; border: 1.5px solid #e8e8e8; transition: all .2s; flex: 1; min-width: 0; user-select: none; }
.exe-cam-chip:hover { border-color: #bae7ff; background: #f6fbff; }
.exe-cam-chip.active { background: linear-gradient(135deg, #e6f7ff, #bae7ff); border-color: #1890ff; color: #096dd9; font-weight: 600; box-shadow: 0 1px 4px rgba(24,144,255,.2); }
.exe-cam-chip.linked { border-color: #1890ff; box-shadow: 0 0 0 2px rgba(24,144,255,.25), 0 1px 4px rgba(24,144,255,.2); }
.exe-cam-chip.offline { background: #f5f5f5; border-color: #e0e0e0; color: #bbb; }
.exe-cam-chip.offline .exe-cam-chip-icon, .exe-cam-chip.offline .exe-cam-chip-name { opacity: 0.45; }
.exe-cam-chip.offline .exe-cam-dot { opacity: 1; }
.exe-cam-chip.offline:hover { background: #f0f0f0; color: #999; }
.exe-cam-chip.offline:hover .exe-cam-chip-icon, .exe-cam-chip.offline:hover .exe-cam-chip-name { opacity: 0.65; }
.exe-cam-dot { width: 8px; height: 8px; border-radius: 50%; background: #d9d9d9; flex-shrink: 0; display: inline-block; }
...
.exe-cam-more { display: inline-flex; align-items: center; gap: 4px; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 13px; color: #666; background: #fafafa; border: 1.5px dashed #d9d9d9; transition: all .2s; flex-shrink: 0; white-space: nowrap; }
...
.cam-drop-item { display: flex; align-items: center; gap: 8px; padding: 6px 10px; border-radius: 6px; cursor: pointer; font-size: 13px; color: #555; transition: background .15s; }
...
.exe-grid.g-6 { grid-template-columns: 1fr 1fr 1fr; grid-template-rows: 1fr 1fr 1fr; grid-template-areas: 'a a b' 'a a c' 'd e f'; }
.exe-grid.g-8 { grid-template-columns: 1fr 1fr 1fr 1fr; grid-template-rows: 1fr 1fr 1fr 1fr; grid-template-areas: 'a a a b' 'a a a c' 'a a a d' 'e f g h'; }
.exe-grid.g-7 { grid-template-columns: 1fr 1fr 1fr 1fr; grid-template-rows: 1fr 1fr 1fr 1fr; grid-template-areas: 'a a c c' 'a a c c' 'b b d e' 'b b f g'; }
...
.exe-cell-offline-msg { color: #ff4d4f; font-size: 13px; font-weight: 600; letter-spacing: 1px; margin-bottom: 2px; }
.exe-cell-empty { color: #555; font-size: 13px; }
.exe-cell-video { position: absolute; inset: 0; background: radial-gradient(ellipse at 30% 40%, #1e2a3a 0%, #0d1117 60%, #0a0e14 100%); display: flex; align-items: flex-end; }
.exe-cell-scanline { position: absolute; inset: 0; background: repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(255,255,255,.008) 2px, rgba(255,255,255,.008) 4px); animation: scanMove 4s linear infinite; pointer-events: none; }
@keyframes scanMove { 0% { transform: translateY(0); } 100% { transform: translateY(4px); } }
.exe-cell-overlay { position: absolute; inset: 0; display: flex; flex-direction: column; justify-content: flex-end; padding: 8px 10px; background: linear-gradient(to top, rgba(0,0,0,.7) 0%, transparent 50%); pointer-events: none; }
.exe-cell-rec { color: #ff4d4f; font-size: 10px; font-weight: 600; letter-spacing: 1px; animation: recBlink 1s step-end infinite; margin-bottom: 2px; }
...
.exe-cell-close { position: absolute; top: 0; right: 0; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 2; opacity: 0; transition: opacity .2s; }
.exe-cell:hover .exe-cell-close { opacity: 1; }
.exe-cell-close-icon { width: 20px; height: 20px; border-radius: 50%; background: rgba(0,0,0,.6); color: rgba(255,255,255,.7); font-size: 14px; line-height: 20px; text-align: center; transition: all .2s; }
...
.split-btn-pv { display: inline-grid; width: 16px; height: 16px; gap: 1px; vertical-align: middle; }
.split-btn-pv-cell { background: #bbb; border-radius: 1px; min-width: 0; min-height: 0; }
.split-menu { display: grid; grid-template-columns: repeat(4, 72px); gap: 6px; padding: 8px; }
.split-card { display: flex; flex-direction: column; align-items: center; gap: 5px; padding: 8px 4px 6px; border-radius: 6px; cursor: pointer; border: 2px solid transparent; transition: all .2s; }
.split-card:hover { background: #f0f5ff; border-color: #d6e4ff; }
.split-card.active { background: #e6f7ff; border-color: #1890ff; }
.split-pv { display: grid; width: 48px; height: 48px; gap: 2px; }
.split-pv-cell { background: #d9d9d9; border-radius: 2px; min-width: 0; min-height: 0; }
...
.split-pv.pv-6 { grid-template-columns:1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr; grid-template-areas:'a a b' 'a a c' 'd e f'; }
...
.split-pv.pv-8 { grid-template-columns:1fr 1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr 1fr; grid-template-areas:'a a a b' 'a a a c' 'a a a d' 'e f g h'; }
...
.split-pv.pv-7 { grid-template-columns:1fr 1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr 1fr; grid-template-areas:'a a c c' 'a a c c' 'b b d e' 'b b f g'; }
...
.split-btn-pv.pv-6 { grid-template-columns:1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr; grid-template-areas:'a a b' 'a a c' 'd e f'; }
...
.split-btn-pv.pv-8 { grid-template-columns:1fr 1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr 1fr; grid-template-areas:'a a a b' 'a a a c' 'a a a d' 'e f g h'; }
...
.split-btn-pv.pv-7 { grid-template-columns:1fr 1fr 1fr 1fr; grid-template-rows:1fr 1fr 1fr 1fr; grid-template-areas:'a a c c' 'a a c c' 'b b d e' 'b b f g'; }
...
.split-card-label { font-size: 12px; color: #666; line-height: 1; white-space: nowrap; }
...
.exe-empty { text-align: center; padding: 40px; color: #999; font-size: 14px; }
