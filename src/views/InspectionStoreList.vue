...
// ========== 门店卡片 ==========
const mockStores = [
  { id:'s1',name:'21世纪太阳城xxx商铺',hasDevice:true,taskId:'T200-P111-xxxA',regionPath:['root','huabei','nj','qb'] },
  { id:'s2',name:'xxx万达苏宁--xxx旗舰门店',hasDevice:true,taskId:'T200-P111-xxxB',regionPath:['root','huabei','nj','qb'] },
  { id:'s3',name:'xxx万达苏宁--xxx旗舰门店',hasDevice:true,taskId:'T200-P111-xxxC',regionPath:['root','huanan','gz','th'] },
  { id:'s4',name:'xxx万达苏宁--xxx旗舰门店',hasDevice:false,taskId:'',regionPath:['root','huadong','sh','lujz'] },
  { id:'s5',name:'21世纪太阳城xxx商铺',hasDevice:true,taskId:'T200-P111-xxxD',regionPath:['root','huabei','nj','xl'] },
  { id:'s6',name:'xxx万达苏宁--xxx旗舰门店',hasDevice:true,taskId:'T200-P111-xxxE',regionPath:['root','huanan','gz','th'] },
  { id:'s7',name:'xxx万达苏宁--xxx旗舰门店',hasDevice:true,taskId:'T200-P111-xxxF',regionPath:['root','huabei','nj','qb'] },
]

// 在线巡检仅展示有待执行任务的门店（hasDevice=true）
const pendingStores = computed(() => mockStores.filter(s => s.hasDevice))

// 动态计算每个树节点的待执行门店数
const storeCountByKey = computed(() => {
  const map: Record<string, number> = {}
  for (const s of pendingStores.value) {
...
const filteredStores = computed(() => {
  const list = pendingStores.value
  if (!selectedKey.value || selectedKey.value === 'root') return list
  return list.filter(s => s.regionPath.includes(selectedKey.value))
})

const goExecute = (store: any) => {
...
<template>
  <div class="sl-page">
    ...
    <div class="sl-content">
      <a-alert type="info" show-icon class="sl-alert">
        <template #message>
          在线巡检页面仅显示当前有待执行巡检任务的门店，如需突击点检巡查可去
          <a @click="goSpotCheck" class="sl-link">视频点检</a>。
        </template>
      </a-alert>
      <div class="sl-grid" v-if="filteredStores.length > 0">
        <div v-for="store in filteredStores" :key="store.id" class="sl-card">
          ...
          <div class="slc-actions">
            <a-button type="primary" :disabled="!store.hasDevice" @click="goExecute(store)">立即巡检</a-button>
            <a-button @click="viewHistory(store)">查看记录</a-button>
          </div>
        </div>
      </div>
      <div v-else class="sl-empty">
        <a-empty description="该区域暂无待执行巡检任务的门店" />
      </div>
...
.sl-alert { margin-bottom: 20px; border-radius: 6px; }
.sl-link { color: #1890ff; text-decoration: underline; cursor: pointer; }
