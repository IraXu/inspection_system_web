...
const formSnapshotStart = ref<dayjs.Dayjs | string>('')
const formSnapshotEnd = ref<dayjs.Dayjs | string>('')
...
const handleSave = () => {
  ...
  setTimeout(() => {
    ...
    const data: AIInspectionTask = {
      ...
      snapshotStart: typeof formSnapshotStart.value === 'string' ? formSnapshotStart.value : (formSnapshotStart.value as any)?.format?.('HH:mm') || '08:00', snapshotEnd: typeof formSnapshotEnd.value === 'string' ? formSnapshotEnd.value : (formSnapshotEnd.value as any)?.format?.('HH:mm') || '18:00',
      ...
      confidenceThreshold: formConfidenceThreshold.value ?? 80,
  ...
  <div class="ai-task-page">
    ...
    <a-drawer v-model:open="drawerVisible" :title="drawerTitle" width="720px" :closable="true" :mask-closable="false" placement="right" :footer-style="{ textAlign: 'right' }">
      <a-form :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }" :colon="false" class="task-form">
        ...
        <a-form-item label="置信度阈值" required>
          <div class="confidence-inline">
            <span class="confidence-prefix">AI分析检测项置信度 ≥</span>
            <a-input-number v-model:value="formConfidenceThreshold" :min="0" :max="100" :step="5" placeholder="范围0~100" style="width:110px" />
            <span class="confidence-suffix">% 时告警</span>
          </div>
        </a-form-item>
...
/* 置信度 */
.confidence-inline { display: flex; align-items: center; gap: 8px; }
.confidence-prefix, .confidence-suffix { font-size: 13px; color: #333; }
