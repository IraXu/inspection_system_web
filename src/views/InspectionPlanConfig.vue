...
const mockPlans: InspectionPlan[] = [
  ...
  { id: '4', name: '计划D-单次专项检查', status: 'ended', inspectionMode: 'offline', description: '已结束的单次检查', cycleMode: 'once', executionTime: '2026-01-15 10:00', storesPersonnelTemplates: [{ key: '1', storeId: 's1', storeName: '门店A', personnelIds: ['p1','p4'], personnelNames: ['张三','赵六'], templateId: 't2', templateName: '明星店铺综合巡检' }], creator: '管理员', createdAt: '2026-01-01 08:00:00', updatedAt: '2026-02-01 08:00:00' },
...
const handleSave = () => {
  ...
  if(formCycleMode.value==='once'){
    ...
    if(!formOnceValidRange.value){message.warning('请选择任务有效期');return}
    if(!formOnceDuration.value||formOnceDuration.value<=0){message.warning('请输入有效的任务时效');return}
  }else{
    ...
    if(!formCycleValidRange.value){message.warning('请选择任务有效期');return}
    if(!formCycleDuration.value||formCycleDuration.value<=0){message.warning('请输入有效的任务时效');return}
  }
  ...
  if(formCycleMode.value==='once'){ data.executionTime=`${formOnceGenDate.value} ${formOnceGenTime.value}`;data.validUntil=formOnceValidRange.value?.[1]||'';data.duration=`${formOnceDuration.value}小时` }
    ...
    <a-drawer v-model:open="drawerVisible" :title="drawerMode==='add'?'新增巡检计划':'编辑巡检计划'" size="large" :styles="{body:{paddingBottom:'80px'}}">
      <a-form :label-col="{span:5}" :wrapper-col="{span:17}">
        ...
        <template v-if="formCycleMode==='once'">
          ...
          <a-form-item label="任务有效期" required><a-range-picker v-model:value="formOnceValidRange" style="width:320px" /></a-form-item>
          <a-form-item label="任务时效" required>
            ...
            <div style="color:#999;font-size:12px;margin-top:4px">巡检人需在截止时间内完成任务</div>
          </a-form-item>
        </template>
        ...
        <template v-if="formCycleMode==='cycle'">
          ...
          <a-form-item label="任务有效期" required><a-range-picker v-model:value="formCycleValidRange" style="width:320px" /></a-form-item>
          <a-form-item label="任务时效" required>
    ...
    <a-modal v-model:open="detailVisible" title="巡检计划详情" :footer="null" width="600px">
      <template v-if="detailPlan">
        <a-descriptions :column="2" bordered size="small">
          ...
          <a-descriptions-item label="任务有效期">{{detailPlan.validUntil||'-'}}</a-descriptions-item>
          <a-descriptions-item label="任务时效">{{detailPlan.duration||'-'}}</a-descriptions-item>
